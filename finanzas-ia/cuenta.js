...
(function () {
  ...
  function pintar(u) {
    ...
    $("#stat-usados").textContent = (u.historial || []).filter(h => h.modulo && h.modulo[0] !== "_").length;
  ...
  function pintarHistorial(u) {
    ...
    host.innerHTML = items.map(h => `
      <details class="historial-item">
        <summary><span>${escHTML(h.modulo_nombre || h.modulo)}</span><span class="fecha">${formatearFechaHora(h.fecha)}</span></summary>
  ...
  async function cancelarPlan() {
    if (!confirm("¿Seguro que quieres cancelar? Volverás al plan Gratis (5 análisis/mes).")) return;
